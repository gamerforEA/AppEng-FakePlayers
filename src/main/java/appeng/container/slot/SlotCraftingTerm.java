...
public class SlotCraftingTerm extends AppEngCraftingSlot
{
	...
	public void doClick(final InventoryAction action, final EntityPlayer who)
	{
		...
		maxTimesToCraft = Math.min(maxTimesToCraft, EventConfig.maxTimesToCraft);
	...
	private ItemStack craftItem(final EntityPlayer player, final ItemStack request, final IMEMonitor<IAEItemStack> inv, final IItemList all)
	{
		// update crafting matrix...
		ItemStack craftingResult = this.getStack();

		if (craftingResult != null && Platform.isSameItem(request, craftingResult))
		{
			IInventory pattern = this.getPattern();
			final ItemStack[] set = new ItemStack[pattern.getSizeInventory()];
			...
			if (Platform.isServer())
			{
				...
				final IRecipe recipe = Platform.findMatchingRecipe(ic, player.worldObj);
				...
				if (recipe == null)
				{
					...
					if (target.isDamageable() && target.isRepairable())
					{
						...
						if (!isBad)
						{
							super.onPickupFromSlot(player, craftingResult);
				...
				craftingResult = recipe.getCraftingResult(ic);

				if (inv != null)
					for (int slot = 0; slot < pattern.getSizeInventory(); slot++)
					{
						ItemStack patternStack = pattern.getStackInSlot(slot);
						if (patternStack != null)
						{
							set[slot] = Platform.extractItemsByRecipe(this.energySrc, this.mySrc, inv, player.worldObj, recipe, craftingResult, ic, patternStack, slot, all, Actionable.MODULATE, ItemViewCell.createFilter(this.container.getViewCells()));
							ic.setInventorySlotContents(slot, set[slot]);
						}
					}
			}

			if (this.preCraft(player, inv, set, craftingResult))
			{
				this.makeItem(player, craftingResult);

				this.postCraft(player, inv, set, craftingResult);
			}
