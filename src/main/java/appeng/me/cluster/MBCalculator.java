...
public abstract class MBCalculator
{
	...
	public void calculateMultiblock(final World world, final WorldCoord loc)
	{
		...
		try
		{
			...
			while (this.checkXZ() && this.isValidTileAt(world, min.x - 1, min.y, min.z))
			{
				min.x--;
			}
			// TODO gamerforEA add condition [1]
			while (this.checkY() && this.isValidTileAt(world, min.x, min.y - 1, min.z))
			{
				min.y--;
			}
			// TODO gamerforEA add condition [1]
			while (this.checkXZ() && this.isValidTileAt(world, min.x, min.y, min.z - 1))
			{
				min.z--;
			}
			// TODO gamerforEA add condition [1]
			while (this.checkXZ() && this.isValidTileAt(world, max.x + 1, max.y, max.z))
			{
				max.x++;
			}
			// TODO gamerforEA add condition [1]
			while (this.checkY() && this.isValidTileAt(world, max.x, max.y + 1, max.z))
			{
				max.y++;
			}
			// TODO gamerforEA add condition [1]
			while (this.checkXZ() && this.isValidTileAt(world, max.x, max.y, max.z + 1))
			{
				max.z++;
			}
	...
	private boolean verifyUnownedRegionInner(final World w, int minX, int minY, int minZ, int maxX, int maxY, int maxZ, final ForgeDirection side)
	{
		switch (side)
		{
			case WEST:
