...
public class CellInventory implements ICellInventory
{
	...
	private static boolean isStorageCell(final Item type)
	{
		try
		{
			if (type instanceof IStorageCell)
	...
	private static boolean isEmptyCell(final IAEItemStack itemStack)
	{
		if (!itemStack.hasTagCompound())
			return true;

		final IAETagCompound tagCompound = itemStack.getTagCompound();
		if (!(tagCompound instanceof AESharedNBT))
		{
			final IMEInventory<IAEItemStack> meInventory = getCell(itemStack.getItemStack(), null);
			return meInventory == null || meInventory.getAvailableItems(AEApi.instance().storage().createItemList()).isEmpty();
		}

		final AESharedNBT sharedNBT = (AESharedNBT) tagCompound;
		final int types = sharedNBT.getInteger(ITEM_TYPE_TAG);
		if (types <= 0)
			return true;

		for (int x = 0; x < types; x++)
		{
			String countTagName = itemSlotCount == null ? ITEM_SLOT_COUNT + x : itemSlotCount[x];
			int stackSize = sharedNBT.getInteger(countTagName);
			if (stackSize > 0)
				return false;
		}
	...
	public IAEItemStack injectItems(final IAEItemStack input, final Actionable mode, final BaseActionSource src)
	{
		...
		final IAEItemStack sharedItemStack = input.copy();
		if (CellInventory.isStorageCell(sharedItemStack) && !isEmptyCell(sharedItemStack))
			return input;
		// TODO gamerforEA code end
		...
		if (this.canHoldNewItem()) // room for new type, and for at least one item!
		{
			...
			if (remainingItemCount > 0)
			{
				if (input.getStackSize() > remainingItemCount)
				{
					...
					final IAEItemStack toReturn = sharedItemStack.copy();
					toReturn.setStackSize(sharedItemStack.getStackSize() - remainingItemCount);

					if (mode == Actionable.MODULATE)
					{
						final IAEItemStack toWrite = sharedItemStack.copy();
