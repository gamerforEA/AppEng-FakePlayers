...
public class EnergyGridCache implements IEnergyGrid
{
	...
	public double extractAEPower(final double amt, final Actionable mode, final PowerMultiplier pm)
	{
		...
		return pm.divide(this.extractAEPower(pm.multiply(amt), mode, this.localSeen, EventConfig.speculativeOptimizations));
	...
	@Override
	public double extractAEPower(final double amt, final Actionable mode, final Set<IEnergyGrid> seen) */
	private double extractAEPower(final double amt, final Actionable mode, final Set<IEnergyGrid> seen, final boolean firstCall)
	// TODO gamerforEA code end
	{
		/* TODO gamerforEA code replace, old code:
		if (!seen.add(this))
			return 0; */
		final Collection<IEnergyGridProvider> energyGridProviders = EventConfig.optimizeEnergyGridCache ? this.energyGridProviders.elementSet() : this.energyGridProviders;
		if ((!firstCall || !energyGridProviders.isEmpty()) && !seen.add(this))
			return 0;
		// TODO gamerforEA code end
		...
		if (mode == Actionable.SIMULATE)
		{
			...
			if (extractedPower < amt)
			{
				...
				final Iterator<IEnergyGridProvider> i = energyGridProviders.iterator();
				// TODO gamerforEA code end

				while (extractedPower < amt && i.hasNext())
				{
					IEnergyGridProvider node = i.next();
					...
					extractedPower += node.extractAEPower(amt - extractedPower, mode, seen);
				}
			}
		...
		if (extractedPower < amt)
		{
			...
			while (extractedPower < amt && i.hasNext())
			{
				final IEnergyGridProvider node = i.next();
	...
	@Override
	public double injectAEPower(double amt, final Actionable mode, final Set<IEnergyGrid> seen) */
	private double injectAEPower(double amt, final Actionable mode, final Set<IEnergyGrid> seen, final boolean firstCall)
	// TODO gamerforEA code end
	{
		/* TODO gamerforEA code replace, old code:
		if (!seen.add(this))
		...
		if (mode == Actionable.SIMULATE)
		{
			...
			// TODO gamerforEA code replace, old code:
			// final Iterator<IEnergyGridProvider> i = this.energyGridProviders.iterator();
			final Iterator<IEnergyGridProvider> i = amt > 0 ? energyGridProviders.iterator() : Collections.emptyIterator();
			// TODO gamerforEA code end
			...
			while (amt > 0 && i.hasNext())
			{
				...
				amt = node.injectAEPower(amt, mode, seen);
			}
		}
		else
		{
	...
	@Override
	public double getEnergyDemand(final double maxRequired, final Set<IEnergyGrid> seen) */
	private double getEnergyDemand(final double maxRequired, final Set<IEnergyGrid> seen, final boolean firstCall)
	// TODO gamerforEA code end
	{
		/* TODO gamerforEA code replace, old code:
		if (!seen.add(this))
		...
		{
			final Iterator<IEnergyGridProvider> ix = energyGridProviders.iterator();
			while (required < maxRequired && ix.hasNext())
			{
				final IEnergyGridProvider node = ix.next();
				if (node == this)
	...
	public double injectPower(final double amt, final Actionable mode)
	{
		...
		return this.injectAEPower(amt, mode, this.localSeen, EventConfig.speculativeOptimizations);
		// TODO gamerforEA code end
	}
	...
	public double getEnergyDemand(final double maxRequired)
	{
		...
		return this.getEnergyDemand(maxRequired, this.localSeen, EventConfig.speculativeOptimizations);
		// TODO gamerforEA code end
	}
	...
	public void removeNode(final IGridNode node, final IGridHost machine)
	{
		...
		{
			final IEnergyWatcher myWatcher = this.watchers.remove(node);
			if (myWatcher != null)
				myWatcher.clear();
		}
		// TODO gamerforEA code end
