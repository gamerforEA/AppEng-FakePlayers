...
public class ToolNetworkTool extends AEBaseItem implements IGuiItem, IAEWrench, IToolWrench
{
	...
	public ItemStack onItemRightClick(ItemStack it, World w, EntityPlayer p)
	{
		if (Platform.isClient())
		{
			...
			else
			{
				...
				if (w.getBlock(i, j, k).isAir(w, i, j, k)) onItemUseFirst(it, p, w, 0, 0, 0, -1, 0, 0, 0);
			}
		}
	...
	public boolean onItemUseFirst(ItemStack is, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ)
	{
		...
		if (te instanceof IPartHost)
		{
			...
			if (part.part != null)
			{
				if (part.part instanceof INetworkToolAgent && !((INetworkToolAgent) part.part).showNetworkInfo(mop)) return false;
			}
		}
	...
	public boolean serverSideToolLogic(ItemStack is, EntityPlayer p, World w, int x, int y, int z, int side, float hitX, float hitY, float hitZ)
	{
		if (side >= 0)
		{
			...
			if (!p.isSneaking())
			{
				if (p.openContainer instanceof AEBaseContainer) return true;

				TileEntity te = w.getTileEntity(x, y, z);

				if (te instanceof IGridHost) Platform.openGUI(p, te, ForgeDirection.getOrientation(side), GuiBridge.GUI_NETWORK_STATUS);
				else Platform.openGUI(p, null, ForgeDirection.UNKNOWN, GuiBridge.GUI_NETWORK_TOOL);

				return true;
			}
			else b.onBlockActivated(w, x, y, z, p, side, hitX, hitY, hitZ);
		}
		else Platform.openGUI(p, null, ForgeDirection.UNKNOWN, GuiBridge.GUI_NETWORK_TOOL);
