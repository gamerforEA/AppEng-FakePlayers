...
public final class AECommand extends CommandBase
{
		...
		if ("help".equals(args[0]))
			try
			{
				if (args.length > 1)
				{
					...
					final Commands c = Commands.valueOfIgnoreCase(args[1]);
			...
			try
			{
				...
				final Commands c = Commands.valueOfIgnoreCase(args[0]);
				// TODO gamerforEA code end
	...
	private static final ImmutableList<String> REAL_SUB_COMMANDS;

	@Override
	public List addTabCompletionOptions(ICommandSender sender, String[] args)
	{
		if (args.length == 1)
			return getListOfStringsFromIterableMatchingLastWord(args, SUB_COMMANDS);
		else if (args.length > 1)
			return getListOfStringsFromIterableMatchingLastWord(args, REAL_SUB_COMMANDS);
		return null;
	}

	static
	{
		REAL_SUB_COMMANDS = ImmutableList.copyOf(Arrays.stream(Commands.values()).map(Enum::name).collect(Collectors.toList()));

		ImmutableList.Builder<String> builder = ImmutableList.builder();
		builder.add("list");
		builder.add("help");
		builder.addAll(REAL_SUB_COMMANDS);
